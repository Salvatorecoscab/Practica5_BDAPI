...
try {
  if (!admin.apps.length) {
    // Inicializar explícitamente con la databaseURL
    admin.initializeApp({
      databaseURL: "https://crudfirebaseapp-2908f-default-rtdb.firebaseio.com"
    });
    logger.info("Firebase Admin SDK initialized with explicit databaseURL.");
  }
} catch (e) {
...
functions.http("sendHttpPushNotification", async (req, res) => {
  ...
  try {
    const userAdminSnapshot = await admin.database() // Esta llamada ahora debería funcionar
  ...
  if (!targetToken || !notificationTitle || !notificationBody) {
    ...
    error: "Bad Request: Missing targetToken, notification.title, or notification.body."});
    return;
  }

  logger.info(`Admin ${callingUserId} sending notification. Title: "${notificationTitle}", Body: "${notificationBody}", To Token: ${targetToken ? targetToken.substring(0, 20) : 'N/A'}...`);
  ...
  try {
    ...
    if (error.code === "messaging/invalid-registration-token" ||
      ...
      res.status(500).send({success: false,
      error: `Failed to send notification: ${error.message}`, errorCode: error.code});
    }
  }
});
